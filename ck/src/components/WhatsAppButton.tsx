...
export function WhatsAppButton({
  ...
  message: string;
  /** Optional explicit link (overrides the WhatsApp deep link). */
  href?: string;
  size?: "sm" | "md" | "lg";
  className?: string;
  ...
  return (
    <a
      href={href || wa(message)}
      target="_blank"
      rel="noopener noreferrer"
      className={`group inline-flex items-center gap-2 rounded-full bg-whatsapp font-semibold text-white shadow-soft transition-all duration-300 hover:-translate-y-0.5 hover:bg-whatsapp-dark hover:shadow-card ${sizes[size]} ${className}`}
    >
...
export function FloatingWA() {
  ...
  return (
    <a
      ...
      className="fixed bottom-5 right-5 z-50 grid h-14 w-14 place-items-center rounded-full bg-whatsapp text-white shadow-glow transition-transform duration-300 hover:scale-110 animate-float-slow"
    >
